...
function graphFileCandidates(fileName: string): string[] {
  ...
  const projectDir = process.env.ARCH_PROJECT_DIR;
  const projectId = process.env.ARCH_PROJECT_ID;
  const repoId = process.env.ARCH_REPO_ID;
  const cwd = process.cwd();
  const projectCandidates =
    projectId && repoId
      ? [
          path.resolve(cwd, `.understand-arch/${projectId}/specs/repos/${repoId}/${fileName}`),
          path.resolve(cwd, `.understand-arch/${projectId}/specs/${fileName}`),
        ]
      : [];
  return [
    ...(graphDir ? [path.resolve(graphDir, fileName)] : []),
    ...(projectDir ? [path.resolve(projectDir, "specs", fileName)] : []),
    ...(projectDir && repoId ? [path.resolve(projectDir, "specs", "repos", repoId, fileName)] : []),
    ...projectCandidates,
    path.resolve(cwd, `.understand-arch/${fileName}`),
    path.resolve(cwd, `../../../.understand-arch/${fileName}`),
  ];
}
...
function projectRootFromGraphFile(candidate: string): string {
  ...
  const marker = parts.lastIndexOf(".understand-arch");
  if (marker > 0) {
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "@understand-arch/core/schema": path.resolve(__dirname, "../engine/core/dist/schema.js"),
      "@understand-arch/core/search": path.resolve(__dirname, "../engine/core/dist/search.js"),
      "@understand-arch/core/types": path.resolve(__dirname, "../engine/core/dist/types.js"),
    },
  },
      ...
      configureServer(server) {
        ...
        server.middlewares.use((req, res, next) => {
          ...
          if (pathname === "/config.json") {
            ...
            sendJson(res, 200, { hooks_enabled: false, outputLanguage: "en" });
            return;
          }
          ...
          for (const candidate of candidates) {
            ...
            try {
              ...
              console.error("[understand-arch] Failed to sanitise graph file:", err);
              res.statusCode = 500;
              res.setHeader("Content-Type", "application/json");
