...
import { validateGraph } from "@understand-arch/core/schema";
import type { GraphIssue } from "@understand-arch/core/schema";
import { useDashboardStore } from "./store";
import GraphView from "./components/GraphView";
import DomainGraphView from "./components/DomainGraphView";
import ArchitectureLayerView from "./components/ArchitectureLayerView";
...
const SESSION_TOKEN_KEY = "understand-arch-token";
...
function dataUrl(fileName: string, token: string | null): string {
  if (DEMO_MODE) {
    const envMap: Record<string, string | undefined> = {
      ...
      "arch-layer.json": import.meta.env.VITE_ARCH_LAYER_URL,
...
function Dashboard({ accessToken }: { accessToken: string }) {
  ...
  const setArchLayer = useDashboardStore((s) => s.setArchLayer);
    ...
    fetch(dataUrl("knowledge-graph.json", accessToken))
      ...
      .then((data: unknown) => {
        ...
        if (result.success && result.data) {
          ...
          setGraphIssues(result.issues);for (const issue of result.issues) {
  ...
  useEffect(() => {
    fetch(dataUrl("arch-layer.json", accessToken))
      .then((res) => {
      ...
      .then((data: unknown) => {
        if (data && typeof data === "object") {
          setArchLayer(data as Parameters<typeof setArchLayer>[0]);
        }
      })
      .catch(() => {});
  }, [setArchLayer]);
