 ...
 *
 * Input:  <project-root>/.understand-arch/intermediate/scan-result.json
 * Output: <project-root>/.understand-arch/intermediate/batches.json
 */
...
try {
  ...
  core = await import(pathToFileURL(resolve(PLUGIN_ROOT, 'engine/core/dist/index.js')).href);
}
...
async function main() {
  ...
  const scanPath = join(archProjectDir(projectRoot), 'intermediate', 'scan-result.json');
  ...
  const outPath = join(archProjectDir(projectRoot), 'intermediate', 'batches.json');
