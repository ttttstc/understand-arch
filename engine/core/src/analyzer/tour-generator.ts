...
export function buildTourGenerationPrompt(graph: KnowledgeGraph): string {
  ...
  return `You are a senior software architect. Generate an architecture tour of the following project that helps engineers understand the system shape, capability support, critical flows, and risk-bearing components step by step.
...
1. Starts with the system overview and entry points
2. Follows the natural dependency and runtime flow
3. Groups related files by architectural responsibility
4. Calls out critical boundaries, data movement, or operational concerns when visible in the graph
5. Ends with supporting utilities or architectural concepts
