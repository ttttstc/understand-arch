<#
.SYNOPSIS
  understand-arch installer for Windows (PowerShell).
...
$RepoUrl    = if ($env:UA_REPO_URL) { $env:UA_REPO_URL } else { 'https://github.com/ttttstc/understand-arch.git' }
$RepoDir    = if ($env:UA_DIR)      { $env:UA_DIR }      else { Join-Path $HOME '.understand-arch\repo' }
$PluginLink = Join-Path $HOME '.understand-arch-plugin'
...
function Show-Usage {
    @"
understand-arch installer (Windows)
...
Environment:
  ...
  UA_DIR        Override clone destination (default: %USERPROFILE%\.understand-arch\repo)
"@
}
...
function Link-Skills([string]$Target, [string]$Style) {
    ...
    switch ($Style) {
        ...
        'folder' {
            $link = Join-Path $Target 'understand-arch'
...
function Unlink-Skills([string]$Target, [string]$Style) {
    ...
    switch ($Style) {
        ...
        'folder' {
            Remove-Reparse (Join-Path $Target 'understand-arch') | Out-Null
        }
    }
}
...
function Cmd-Install([string]$Id) {
    ...
    Write-Host "`n✓ Installed understand-arch for $Id"
