...
link_skills() {
  ...
  case "$style" in
    per-skill)
      ...
      ;;
    folder)
      ln -sfn "$root" "$target/understand-arch"
      printf '  ✓ %s → %s\n' "$target/understand-arch" "$root"
      ;;
    *)
    ...
    per-skill)
      if [[ -d "$(skills_root)" ]]; then
        ...
        for link in "$target"/*; do
          ...
          [[ "$resolved" == *"/skills/"* ]] || continue
          rm -f "$link"
        done
      fi
      ;;
    folder)
      [[ -L "$target/understand-arch" ]] && rm -f "$target/understand-arch"
      ;;
  esac
}
...
cmd_install() {
  ...
  printf '\n✓ Installed understand-arch for %s\n' "$id"
