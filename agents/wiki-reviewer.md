...
Rule 046: Wiki pages must not render evidence in prose or at chapter ends.
Rule 046a: Reject any inline `[evidence:]` marker.
Rule 046b: Reject any `## 证据来源` table.
Rule 046c: Evidence review must read `arch-layer.json#evidence_refs`, not wiki text.
...
Rule 058c: Enforce F3 evidence rule: wiki must show no evidence markers or evidence tables; verify evidence only from arch-layer evidence_refs.
...
Rule 060: Newcomer audience needs clear project context, but not reading-path or glossary meta sections.
Rule 060a: Reject methodology sections that teach how to read the report instead of describing the project.
Rule 060b: Reject wiki pages that explain generic field meanings such as capability, component, boundary, status, maturity, or risk.
...
Rule 084a: Check Q8 no-meta-narrative: reject discussion of how to use the tool, how to read the document, scan counts, internal phases, field meanings, or analyzer limitations.
Rule 084b: Check that friendliness comes from clear project-specific prose, not from methodology or glossary filler.
