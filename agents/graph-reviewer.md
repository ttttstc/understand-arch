...
```bash
node $ARCH_PROJECT_DIR/tmp/ua-graph-validate.js "<graph-file-path>" "$ARCH_PROJECT_DIR/tmp/ua-review-results.json"
```
...
After the script completes, read `$ARCH_PROJECT_DIR/tmp/ua-review-results.json`. Do NOT re-read the original graph file -- trust the script's results entirely.
...
1. Write the JSON to: `<arch-project-dir>/intermediate/review.json`
