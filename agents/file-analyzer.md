...
Execute the pre-built structural extraction script bundled with the understand-arch plugin. This script uses tree-sitter for code files and specialized parsers for non-code files, providing deterministic, high-quality structural extraction without writing any ad-hoc scripts.
...
```bash
cat > $ARCH_PROJECT_DIR/tmp/ua-file-analyzer-input-<batchIndex>.json << 'ENDJSON'
{
...
node <SKILL_DIR>/extract-structure.mjs \
  $ARCH_PROJECT_DIR/tmp/ua-file-analyzer-input-<batchIndex>.json \
  $ARCH_PROJECT_DIR/tmp/ua-file-extract-results-<batchIndex>.json
```
...
After the script returns, verify the output file exists and is non-empty (e.g. `test -s $ARCH_PROJECT_DIR/tmp/ua-file-extract-results-<batchIndex>.json`). Exit 0 with a missing output file means the bundled script silently no-opped — report this as a hard failure rather than proceeding to Step 3.

### Step 3 — Read the extraction results

Read `$ARCH_PROJECT_DIR/tmp/ua-file-extract-results-<batchIndex>.json`. The output format is:
...
**Step B — Decide split.**
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to `.understand-arch/intermediate/batch-<batchIndex>.json`. Done. Skip to Step F.
...
Write part `k` (1-indexed) to `.understand-arch/intermediate/batch-<batchIndex>-part-<k>.json`. Each part is a valid GraphFragment: `{ "nodes": [...], "edges": [...] }`.
