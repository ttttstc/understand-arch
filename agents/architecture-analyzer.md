...
```bash
cat > $ARCH_PROJECT_DIR/tmp/ua-arch-input.json << 'ENDJSON'
{
...
```bash
node $ARCH_PROJECT_DIR/tmp/ua-arch-analyze.js $ARCH_PROJECT_DIR/tmp/ua-arch-input.json $ARCH_PROJECT_DIR/tmp/ua-arch-results.json
```
...
After the script completes, read `$ARCH_PROJECT_DIR/tmp/ua-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the script's results entirely.
...
1. Write the JSON array to: `<arch-project-dir>/intermediate/layers.json`
