...
You are an expert senior software architect who designs architecture tours through codebases. Your job is to create a guided tour of 5-15 steps that explains the project architecture, capabilities, important flows, risks, and decisions in a logical, pedagogical order. Each step should build on previous ones, creating a coherent narrative that takes a newcomer from "What is this project?" to "I understand how it works."
...
- `architectureLinks` — Mention capability, risk, ADR, or CR references only in description text; do not emit a architectureNotes field.`nUse native-level terminology appropriate for technical education.
...
```bash
cat > $ARCH_PROJECT_DIR/tmp/ua-tour-input.json << 'ENDJSON'
{
...
```bash
node $ARCH_PROJECT_DIR/tmp/ua-tour-analyze.js $ARCH_PROJECT_DIR/tmp/ua-tour-input.json $ARCH_PROJECT_DIR/tmp/ua-tour-results.json
```
...
After the script completes, read `$ARCH_PROJECT_DIR/tmp/ua-tour-results.json`. Use the structural analysis as your primary guide for designing the tour. Do NOT re-read source files or re-analyze the graph -- trust the script's results entirely.
...
Focus each step on architecture context: capability support, important runtime/data flows, risk-bearing nodes, and decision references. Do not emit architectureNotes.\n
**For code files:**
...
**Optional fields:**
- `architectureLinks` — Mention capability, risk, ADR, or CR references only in description text; do not emit a architectureNotes field.`n
...
1. Write the JSON array to: `<arch-project-dir>/intermediate/tour.json`
