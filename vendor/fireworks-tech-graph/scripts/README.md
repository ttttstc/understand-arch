...
`generate-diagram.sh` 会优先调用 cairosvg，缺失时自动回退到 rsvg-convert。完整对比见 [PROMPT.md → SVG → PNG Conversion](../PROMPT.md)。
...
pip install cairosvg
```
脚本会自动优先使用 cairosvg。如果仍需要像素级还原（例如浏览器生成的 SVG），改用 puppeteer（见 PROMPT.md）。
